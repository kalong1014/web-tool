    ...
    public function getPromptList()
    {
        ...
        try {
            $list = Db::name('write_prompts')
                ->where($where)
                ->field('id,topic_id,title,desc,thumb,views,usages,votes,weight,state')
                ->order('weight desc, id asc')
                ->page($page, $pagesize)
    ...
    public function getPrompt()
    {
        ...
        try {
            $info = Db::name('write_prompts')
                ...
                ->field('id,title,topic_id,desc,prompt,thumb,hint,welcome,tips,fake_votes,fake_views,fake_usages,weight')
                ->find();
            if (!$info) {
                return errorJson('没有找到数据，请刷新页面重试');
            }
