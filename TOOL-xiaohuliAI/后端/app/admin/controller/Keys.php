    ...
    public function getKeyList()
    {
        ...
        try {
            $list = Db::name('keys')
                ...
                ->order('id desc')
                ->page($page, $pagesize)
                ->select()->each(function($item) use ($e) {
                    ...
                    if ($item['type'] == 'wenxin') {
                        ...
                        unset($item['key']);
                    }elseif ($item['type'] == 'skychat') {
                        $temp = explode('|', $item['key']);
                        $item['appkey'] = $temp[0];
                        $item['appsecret'] = $temp[1] ?? '';
						if($e==1){
							$item['appkey'] = substr_replace($item['appkey'], '*****', 5, 10);
							$item['appsecret'] = substr_replace($item['appsecret'], '*****', 5, 10);
						}
                        unset($item['key']);
                        ...
                        unset($item['key']);
                    } elseif ($item['type'] == 'shangtang') {
                        $temp = explode('|', $item['key']);
                        $item['accesskeyid'] = $temp[0];
                        $item['accesskeysecret'] = $temp[1] ?? '';
						if($e==1){
							$item['accesskeyid'] = substr_replace($item['accesskeyid'], '*****', 5, 10);
							$item['accesskeysecret'] = substr_replace($item['accesskeysecret'], '*****', 5, 10);
						}
                        unset($item['key']);
                    } else {
						if($e==1){
