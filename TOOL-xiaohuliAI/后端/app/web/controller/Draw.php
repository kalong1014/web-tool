...
class Draw extends Base
{
     ...
     * 提交绘画接口
     */
    public function draw($draw_id = null)
    {
        try {
            #1、
            $user = Db::name('user')
            ...
            // 判断余额
            $price = getFuncPrice(self::$site_id, $user, 'draw');
            ...
            if ($platform == 'mj') {
                ...
                } elseif ($channel == 'duomi') {
                    $notifyUrl = 'https://' . $_SERVER['HTTP_HOST'] . '/api.php/notify/duomi_mj';
                    $duomiSDK = new \ChatGPT\duomi($apikey);
                    $result = $duomiSDK->drawMJ('fast', [
                        'prompt' =>  $options['image'] ? $options['image'] . ' ' . $messageEn : $messageEn,
                        'callback_url' => $notifyUrl
                    ]);
                }
                if ($result['errno']) {
                    $errMsg = $result['message'];
                    if (strpos($errMsg, '超出每月免费额度') !== false) {
                        $this->setKeyStop($channel, $apikey, $errMsg);
                        return $this->draw($draw_id);
                    } else {
            ...
            // 扣费
            if ($price) {
                changeUserBalance(self::$user['id'], -$price, '绘画消费');
            }
    ...
    public function submitOpenaiTask($draw_id = null)
    {
        ...
        // 判断禁言
        if ($user['is_freeze']) {
            return $this->setDrawFail($draw_id, '系统繁忙，请稍后再试');
        }
        // 判断余额
        $price = getFuncPrice(self::$site_id, $user, 'draw');
        if ($price && $user['balance_point'] < $price) {
            return $this->setDrawFail($draw_id, '余额不足，请充值！');
        }
        ...
        // 同步绘画方式
        $aiSetting = getAiSetting(self::$site_id, $channel);
        $ChatGPT = new \ChatGPT\openai($apikey);
        $ChatGPT->setApiHost($aiSetting['diyhost']);
    ...
    public function mjCtrl()
    {
        ...
        } elseif ($draw['channel'] == 'duomi') {
            if (empty($draw['image_id'])) {
                return errorJson('本图片不支持此操作');
            }
            if ($type == 'upscale') {
                $mjAction = 'upsample' . $index;
            } elseif ($type == 'variation') {
                $mjAction = $type . $index;
            }
            $notifyUrl = 'https://' . $_SERVER['HTTP_HOST'] . '/api.php/notify/duomi_mj';
            $duomiSDK = new \ChatGPT\duomi($apikey);
            $result = $duomiSDK->mjCtrl('fast', [
                'action' => $mjAction,
        ...
        if ($result['errno']) {
            if (strpos($result['message'], '超出每月免费额度') !== false) {
                $this->setKeyStop($draw['channel'], $apikey, $result['message']);
            }
        ...
        $draw['message'] = preg_replace('/\s-upsample\d|\s-upscale\d|-variation\d/', '', $draw['message']);
        $draw['message_input'] = preg_replace('/\s-upsample\d|\s-upscale\d|-variation\d/', '', $draw['message_input']);
        $draw['message_en'] = preg_replace('/\s-upsample\d|\s-upscale\d|-variation\d/', '', $draw['message_en']);
    ...
    public function getDrawResult()
    {
        ...
        if ($draw['state'] == 0) {
            ...
            if ($now - $draw['create_time'] > 600) {
                $errMsg = text('图片生成失败');
                // 生成超时退费
                $price = getFuncPrice(self::$site_id, self::$user['id'], 'draw');
                if ($price) {
                    changeUserBalance(self::$user['id'], $price, '绘画失败退费');
                }
                $this->setDrawFail($draw_id, $errMsg);
                return successJson([
                    'state' => 2,
    ...
    private function makePromptEn($platform, $options)
    {
        ...
        if ($platform == 'mj') {
            ...
            $prompt = str_replace('-- no', '--no', $prompt); // 翻译可能会把 --no 转成 -- no
            $prompt = str_replace(' -no', ' --no', $prompt); // 翻译可能会把 --no 转成 -no
            $cmd = $this->makeMjCmd($options);
            $prompt .= $cmd;
        } else {
    ...
    private function makeMjCmd($options)
    {
        ...
        if (!empty($options['v'])) {
            ...
            if (in_array($options['v'], ['5.1', '5.2', '6.0']) && $options['style'] != 'raw') {
                $options['style'] = '';
            }
        }
        if (!empty($options['niji'])) {
            ...
            if ($options['niji'] == '4') {
                $options['style'] = '';
            } elseif ($options['niji'] == '5') {
                if ($options['style'] == 'raw') {
                    $options['style'] = '';
                }
            } elseif ($options['niji'] == '6') {
                if ($options['style'] !== 'raw') {
                    $options['style'] = '';
                }
            }
