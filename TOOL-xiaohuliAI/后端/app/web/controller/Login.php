...
class Login
{
    ...
    public function system()
    {
        ...
        $docsSetting = empty($setting['doc']) ? [] : json_decode($setting['doc'], true);
        ...
        $docsIsOpen = empty($docsSetting['is_open']) ? 0 : 1;
        ...
        $menus = isset($webSetting['menus']) ? $webSetting['menus'] : Config::get('menus.pc');
        foreach ($menus as $k => $v) {
            if (!$v['is_show'] || (isset(${$v['type'] . 'IsOpen'}) && !${$v['type'] . 'IsOpen'})) {
                unset($menus[$k]);
                continue;
            }
            unset($menus[$k]['is_show']);
            if (!$v['is_more']) {
                $menus[$k]['title'] = trim($v['short_title']);
                unset($menus[$k]['desc']);
            }
            unset($menus[$k]['short_title']);
            if ($v['type'] != 'diy') {
                unset($menus[$k]['link']);
            }
        }
    ...
    public function getNotice()
    {
        ...
        $notice = Db::name('notice')
            ...
            ->field('id,type,content,update_time')
