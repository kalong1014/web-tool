...
class Novel extends Base
{
    public function getNovelList()
    {
        ...
        try {
            $list = Db::name('novel')
                ->where($where)
                ->field('novel_id,title,prompt,count_finished,count_total,words,create_time')
                ->order('id desc')
                ->page($page, $pagesize)
    ...
    public function getNovelInfo()
    {
        ...
        try {
            $info = Db::name('novel')
                ...
                ->field('novel_id,ai,title,prompt,overview,sketch,count_finished,count_total,words')
                ->find();
            if (!$info) {
    ...
    public function saveNovelInfo()
    {
        ...
        try {
            ...
            $info = Db::name('novel')
                ...
                ->field('novel_id,ai,title,prompt,overview,sketch,count_finished,count_total,words,create_time')
                ->find();
            $info['create_time'] = date('Y-m-d H:i:s', $info['create_time']);
            return successJson($info, '保存成功');
        } catch (\Exception $e) {
